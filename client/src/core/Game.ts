...
import { NEON_MAP } from '../maps/NeonMap';
...
export class Game {
  ...
  duel: Duel | null = null;
  /** Optional Hardpoint (King of the Hill) controller — null unless launched
   *  from the menu. Created + wired by main.ts; ticked here for the capture +
   *  zone-rotation + bot-replenishment loop. */
  ...
  private tick = (now: number) => {
    ...
    if (this.hardpoint) this.hardpoint.update(dt);  // capture tick + zone rotation
