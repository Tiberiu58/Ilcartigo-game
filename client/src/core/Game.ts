 ...
 *  Duel is a 1v1 gauntlet vs a single escalating opponent; King of the Hill
 *  (koth) is FFA combat with a rotating capture zone overlaid. */
...
export class Game {
  ...
  duel: Duel | null = null;
  /** Optional King-of-the-Hill (Hardpoint) controller — null unless launched
   *  from the menu. Created + wired by main.ts; ticked here for the zone loop. */
  ...
  grantStreakReward(streak: number) {
    if (this.mp) return;
    const m = this.mode;
    if (!(m === 'combat' || m === 'tdm' || m === 'onslaught' || m === 'koth')) return;
  ...
  private tick = (now: number) => {
    ...
    if (this.hardpoint) this.hardpoint.update(dt);  // King-of-the-Hill zone loop
