...
> **Status:** v0.41.0 — **🏰 Mansion map overhaul for Heist (Owner vs Thief).** A serious pass on the dedicated **Mansion** map so the asymmetric Thief-vs-Owner minigame feels like a real, replayable environment rather than a box house: a **walled estate** exterior (7 m gate where the Thief spawns, gravel driveway + forecourt, a stone fountain, staggered hedge lanes, trees, bushes, planters, statues — cover-lined approach routes), **six entrances** (front + back doors, two side doors, and two garden French-window flanks on opposite corners), a **two-storey manor** interior (a floor-to-roof **grand hall** with the Owner spawn, flanked by six ground-floor rooms wired so **every room has ≥2 doorways** — full circulation, no dead ends — with furniture-like cover throughout), a **balcony ring + twin staircases + two upstairs perch-rooms + a crouch-peek parapet** over the front door for the Owner's high ground, and a genuinely **reachable cellar vault** (the old map sealed the cellar under one solid ground slab; the slab is now built as a frame that **carves a hole** so the stair-well descends into an enclosed vault room). Night atmosphere (cold moon key vs warm interior point-lights, lit windows) + **teal/gold team spawn pads** for instant readability. Heist wiring upgraded: the opposing-role bots now spawn on the **correct side** (owner-guards inside / thieves at the gate), `VAULT_POS` is **exported from the map and shared with the mode** so objective + room can't drift, and the visible menu is trimmed to exactly **Play / Play Online / Gun Game / Heist** (the other modes stay fully wired in code — the ▾ More Modes drawer toggle is just hidden). All pure-client, headless-clearance-verified, no protocol change. Built on the prior **v0.40.0 — 🏰 Heist mode (Owner vs Thief) + streamlined menu.** A brand-new *asymmetric* mode, very different from the arena loop: pick **Heist** → a side-select (**Thief** vs **Owner**) → the **Mansion** map. The **Owner spawns inside** to defend; the **Thief spawns outside** at the gate, must break in and reach the glowing **vault** in the cellar. The **Owner spawns inside** to defend; the **Thief spawns outside** at the gate, must break in and reach the glowing **vault** in the cellar. v1 skeleton: role-correct spawns, vault objective (thief wins by reaching it / owner by stopping the thief), a role+objective HUD ticker, and a win/lose banner — solo vs the bot roster. New `maps/MansionMap.ts` + `modes/Heist.ts`, `GameMode 'heist'`, `World.removeDecoration`. (Stealable loot, traps, multi-thief later.) Also **streamlined the menu** — only **Play / Play Online / Gun Game / Heist** stay up top; TDM, Onslaught, Duel, Practice, Aim Lab moved into a collapsible **▾ More Modes** drawer (kept intact, just out of the way). Built on **v0.39.0 — Coins, Armory shop & loot crates** (economy round, integrated from the routine — coins + shop + crates only; the routine's parallel Hand-Cannon/coin-pickup/KotH lines were skipped). Includes the **Daily Free Crate** — one free loot crate every day (a `🎁 Claim Free Daily Crate` button + a `· FREE 🎁` badge on the menu Crates button) giving the full shake → rarity-burst → reveal at no Coin cost (`Account.freeCrate`, date-keyed once/day). The core: a second reward economy on top of XP — **Coins**, a soft currency earned by playing (**+3 per kill / +5 on a headshot**, dripped live via the kill bus, plus a **+75 win / +30 top-3** post-match bonus), spent on **loot crates.** A second reward economy on top of XP: **Coins**, a soft currency earned by playing (**+3 per kill / +5 on a headshot**, dripped live in every mode via the kill bus, plus a **+75 win / +30 top-3** post-match bonus), spent on **loot crates** that grant a *random* locked cosmetic with a flashy rarity-tiered reveal. One **600-Coin** crate rolls a Common/Rare/Epic/Legendary drop from the full **59-item** cosmetic pool (skins · kill effects · tracers · finishes), weighted so cheap items come first and the chase tilts toward Legendaries as the collection fills — with **no duplicate drops and no dead rolls** (a complete collection pays Coins instead, and every unlock drops a small `+⛁` shard bonus). New `📦 Crates` main-menu button (live Coin counter), a shop overlay with per-rarity drop odds + collection progress (`X / 59`), and a `+⛁` line on the post-match rewards strip. Pure-client, migration-safe `Account`, **no protocol change**; weapon-mastery skins stay the separate play-to-earn track. New `account/Crates.ts` + `ui/CratesUI.ts`; logic + browser smoke-tested. Built on **v0.38.0 — third routine-integration round + the game has SOUND.** Cherry-picked 11 distinct features off the routine's parallel branches onto current `main`, verifying typecheck + build green after each: **procedural audio engine** (`SynthEngine.ts` synthesizes all 48 SFX at runtime via Web Audio — gunshots, hits, reloads, UI — no `.wav` files; the game was silent before this), **Burst Rifle** (9th weapon, 3-round burst), **Foundry** (7th combat map, molten steelworks), **career achievements + 24 medals** (Awards tab + toasts), **Nemesis system** (marked rival + revenge bonus), **Prestige "Lord" skins** (6000-XP 7th tier), **weapon inspect (T)**, **killfeed weapon icons**, **Final Blow** win cinematic (slow-mo), **hit-combo meter** (x3+ chain counter), and **enemy hit-flash** on landed shots. Skipped the duplicate King-of-the-Hill / neon-map / extra-9th-weapon lines (two branches built them incompatibly); the Crossbow was dropped (heavy conflicts). All v0.34–37 model/animation work (incl. the first-person arms fix) preserved. Built on **v0.37.0 — first-person arms.** The viewmodel now has two low-poly arms whose hands grip the held weapon — a right (trigger) hand + left (foregrip) hand, positioned per-weapon via grip anchors (two-handed near the rear for the pistol). They bob/recoil/swap with the gun and add a **walk/idle sway** of their own — a stronger stride sway that ramps with speed, plus a gentle idle breath when still. Cloak fade includes the hands. Built on **v0.36.0 — animated player character (multiplayer).** Remote MP players now render as a rigged low-poly humanoid (CubeMan) instead of the cyan box-figure, with **driven animations** — Idle when still, Run when moving (from interpolated speed), Death on elimination — via a per-instance `AnimationMixer` and `SkeletonUtils.clone` (the rig must be preserved + properly rebound, unlike the de-rigged guns). Lazy-loaded (FBX + SkeletonUtils in their own chunks; the 3.2 MB model only fetches in MP), with the box figure as a fallback until ready; cloak fade preserved. Bots keep the box figure (per design — characters for *players* only). Built on **v0.35.0 — per-weapon reload animations + weapon colours.** Each weapon now plays a mechanism-appropriate reload motion timed to its real reload window — mag-swap cant/dip (AR · SMG · Marksman · LMG), bolt-cycle jerk (Sniper), double pump-rack (Shotgun), slide pull (Pistol), smooth power-cell tilt (Railgun) — layered into the viewmodel alongside recoil/bob/swap. The FBX weapon exports shipped with all-white materials, so colours are reconstructed from each part's material **name** (LightWood/Metal/Barrels…), plus a per-file index palette for models with generic names (the P90 → dark-olive body). Built on **v0.34.0 — real 3D weapon models**: the first-person viewmodel shows detailed FBX guns (rifle, P90, sniper, shotgun, LMG, ray-gun, pistol) instead of procedural boxes — lazy-loaded, de-rigged (SkinnedMesh→static Mesh so they clone + render), auto-normalized + oriented muzzle-forward, with a graceful box fallback. Built on the v0.33.0 **second routine-integration round.** Two more autonomous build branches merged onto `main`, combining everything they each built. **From the power-ups/progression branch:** **Arena Power-Ups** (OVERCHARGE ×1.7 dmg, RAPID FIRE ×1.55 RoF, OVERSHIELD 50%-absorb — contested buff pads, solo-only), the **Railgun** (8th weapon — piercing beam, one-shot heads, line multi-kills), **Daily Login Rewards** (7-day escalating XP streak), **"ON FIRE" rampage** (persistent killstreak aura), **cosmetics expansion** (kill effects →8, tracers →10, finishes →8), and **skill-shot callouts** (NO SCOPE / AIRBORNE / LONGSHOT). **From the content/feel branch:** **Duel** (solo 1v1 gauntlet — single-elimination ladder vs escalating rivals), **Frostline** (6th combat map, frozen tundra), **weapon identity cards** (archetype + stat bars in the loadout), a **rising hitmarker**, and a **kill banner** ("ELIMINATED {name}"). Their overlapping post-match work was reconciled into one card (accolade + stat strip + NEW PERSONAL BEST). All pure-client / zero-protocol — solo + live MP both intact. Built on the v0.24.0 routine-integration round (Team Deathmatch, Onslaught, Cobalt + Overpass maps, LMG, melee, grenade, nameplates, bot difficulty), the publication round (site + game on Vercel, **MP server live on Fly.io** at `ilcartigo-game.fly.dev`, AdSense `ca-pub-8134911671778438` verified), and Phase 13–14 (Gun Game, Aim Lab, rank ladder, weapon mastery, Marksman, server-authoritative per-weapon damage, minimap, impact FX, health pickups, weapon finishes).
