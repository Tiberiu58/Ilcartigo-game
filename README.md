...
**As of v0.36.0 every sound is synthesized at runtime** (`client/src/audio/SynthEngine.ts`), so the game is fully audible with no files at all. The `.wav` workflow below is now an **optional override** for studio-quality authored audio: drop CC0 `.wav` files into `client/public/assets/sounds/` matching these names, **and list their sound ids in `client/public/assets/sounds/manifest.json`** (a JSON array of ids, e.g. `["fire_ar","hit_confirm"]`). Any id in the manifest loads its `.wav` and takes over from the synth once loaded; anything not in the manifest stays synthesized. With no manifest present (the default) there are zero 404s — pure synth.
