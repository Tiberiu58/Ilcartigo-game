...
## Phase 17 — Personal bests + per-mode records (this round, v0.17.0)

A retention loop for the new modes — **chase your best**. Pure Account + UI, no
gameplay/protocol/MP risk.

- **`Account` extended (migration-safe)** with per-mode `{bestKills, wins,
  plays}`. `recordModeResult()` updates them + returns whether a new best-kills
  record was set; old saves load cleanly and `reset()` wipes it.
- **Post-match**: records the result (online classic keyed separately from solo)
  and flashes a gold "🏆 NEW PERSONAL BEST · N kills" line on a new record.
- **Modes hub cards** show a record line, refreshed on open (Time Attack leads
...
## Phase 21 — Nameplates in multiplayer (this round, v0.21.0)

Completes Phase 20: enemy name + health-bar plates now also render over **remote
players online**, via a new zero-alloc `MultiplayerSession.forEachRemote` visitor
(reads `hp`/`cloaked` off interpolated remote state — no protocol change). Dead
and cloaked remotes show no plate. MP combat now reads as clearly as solo.

## Phase 20 — Enemy nameplates + health bars (v0.20.0)

Combat readability — a floating name + health bar above each enemy so you can
see who you're fighting and how close they are to dying.

- **New `client/src/ui/Nameplates.ts`** — world-space billboarded sprites
  (DamageNumbers pattern), occluded by walls (`depthTest: true`, no wallhack),
  pooled with canvas redraws only on HP change. HP bar ramps green→amber→red;
  fades with distance.
- **Zero coupling into Bot.ts** — pulls live state off `Game.bots` each frame.
  Solo-only v1 (off in MP, where bots are inactive); remote plates can layer on
  later from snapshot data.
