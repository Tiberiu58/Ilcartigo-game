...
> **Status:** v0.42.0 — **Hand Cannon (10th weapon).** A heavy revolver primary: huge per-shot damage (2-shot body, 1-shot head), but a slow 2.2 RPS fire rate, a 6-round cylinder, heavy bloom and a big recoil kick — a high-risk/high-reward precision brawler that punishes a miss with a long recovery, distinct from the light Pistol secondary. MP-safe (a normal single-target hitscan gun, no new mechanic); full integration (config + viewmodel + reload anim + archetype + loadout + Gun Game label + 3 mastery skins + server damage row). Built on **v0.41.0 — reduce-camera-shake setting.** A Settings → General toggle that dampens the screen punch from firing, hits, deaths and explosions to 35% without removing the feedback — broadens the audience (motion sensitivity is a real bounce reason → longer sessions). One choke point: `Game.shakeScale` multiplied into every `applyShake` call. Pure client, persisted, no gameplay impact. Built on **v0.40.0/40.1 — cosmetics expansion + King of the Hill capture juice.** A content drop deepening the unlock chase across four axes (more to grind for → retention → ad impressions), pure data auto-iterated by the Cosmetics UI: +3 kill effects (Ice Nova / Rose Burst / Voltaic → 11 total), +3 weapon finishes (Plasma / Voltline / Obsidian → 11), +2 tracers (Rose / Voltaic → 12), +2 hit markers (Rose / Voltaic → 10). The synthwave-leaning palette ties into the Voltgrid map's neon look. Built on **v0.39.0 — Voltgrid (7th combat map).** A neon-night / synthwave arena: a near-black floor laced with a glowing magenta grid, dark slate structures rimmed in electric-cyan + neon-magenta, translucent neon-glass cover, moody lighting so the emissive accents pop — the Krunker/Tron look, for instant contrast with the warm/rusty/steel/dusk/icy maps. Built on the proven symmetric Cobalt/Frostline skeleton with **byte-for-byte identical collision geometry** (only the non-colliding neon grid is new), so spawn-clearance + TDM fairness carry over with zero risk; the identity is pure palette + lighting. Deepens every solo mode at once (FFA / TDM / Gun Game / Onslaught / Duel / King of the Hill). Solo-selectable, no protocol change; health pads mirrored client ⇆ server. Built on **v0.38.0 — equippable hit markers.** An 8th cosmetic axis on the game's highest-frequency feedback: the X that flashes when you land a hit. 8 unlockable colours (Classic White free → Gold/Cyan/Lime/Magenta/Violet/Amber/Ice, 300–4000 XP), shown on every connecting shot. Only the body-hit colour changes — headshot (red) + kill (glowing red) markers stay fixed so they always read. Exact mirror of the tracer-cosmetic plumbing: `HITMARKERS` registry, migration-safe `Account` state, HUD applies it via a `--hm-color` CSS var, a new "Hit Marker" grid in the Cosmetics tab. Pure-client, no protocol change. Built on **v0.37.0 — Burst Rifle (9th weapon).** A 3-round-burst battle rifle: one trigger pull spits a fast, tight 3-round burst (30 dmg/round) — a clean burst with a head is lethal, pure body needs a follow-up. The skill-gated precision archetype between the full-auto AR and the semi-auto Marksman. The burst is **MP-correct**: a Game-side scheduler fires the 2 follow-up rounds each as a full fire (re-read aim + camera recoil + `sendFire`), so the server registers all three rounds online (single-target/round), not just the first. Additive everywhere — new `WeaponConfig.burst`/`burstDelay` (undefined on every other gun), `BURST_CONFIG`, `buildBurst` viewmodel, archetype + loadout + Gun Game label + 3 mastery skins + server damage row; the two movement controllers untouched. Built on **v0.36.0 — King of the Hill (first objective mode).** A rotating capture-zone hardpoint over the live combat sandbox: stand in the glowing zone **alone** to score (+1 per 0.5 s, first to 100), an enemy bot in it with you turns it amber CONTESTED, a bot holding it alone scores for the enemy. The hardpoint relocates every 42 s (⚑ HARDPOINT MOVED) so nobody camps a corner — constant rotation, constant fights. A 3D zone (translucent disc + pulsing ring + a tall light column visible across the map) recolours live by control (teal you / red enemy / amber contested), a minimap ring mirrors it, ZONE CAPTURED / LOST callouts flash on every flip, and the win ends on a results card (an ad breakpoint) with your-vs-enemy score, captures, bonus XP, NEW PERSONAL BEST + career wins (surfaced on the menu button). Runs on the ordinary FFA bots (kills feed XP/stats/mastery as normal); the only shared-code change is an additive `Bot.objective` attract point (null in every other mode → identical behaviour). Pure-client, no protocol change. Built on **v0.35.0 — per-weapon reload animations + weapon colours.** Each weapon plays a mechanism-appropriate reload motion timed to its real reload window — mag-swap cant/dip (AR · SMG · Marksman · LMG), bolt-cycle jerk (Sniper), double pump-rack (Shotgun), slide pull (Pistol), smooth power-cell tilt (Railgun) — layered into the viewmodel alongside recoil/bob/swap. The FBX weapon exports shipped with all-white materials, so colours are reconstructed from each part's material **name** (LightWood/Metal/Barrels…), plus a per-file index palette for models with generic names (the P90 → dark-olive body). Built on **v0.34.0 — real 3D weapon models**: the first-person viewmodel shows detailed FBX guns (rifle, P90, sniper, shotgun, LMG, ray-gun, pistol) instead of procedural boxes — lazy-loaded, de-rigged (SkinnedMesh→static Mesh so they clone + render), auto-normalized + oriented muzzle-forward, with a graceful box fallback. Built on the v0.33.0 **second routine-integration round.** Two more autonomous build branches merged onto `main`, combining everything they each built. **From the power-ups/progression branch:** **Arena Power-Ups** (OVERCHARGE ×1.7 dmg, RAPID FIRE ×1.55 RoF, OVERSHIELD 50%-absorb — contested buff pads, solo-only), the **Railgun** (8th weapon — piercing beam, one-shot heads, line multi-kills), **Daily Login Rewards** (7-day escalating XP streak), **"ON FIRE" rampage** (persistent killstreak aura), **cosmetics expansion** (kill effects →8, tracers →10, finishes →8), and **skill-shot callouts** (NO SCOPE / AIRBORNE / LONGSHOT). **From the content/feel branch:** **Duel** (solo 1v1 gauntlet — single-elimination ladder vs escalating rivals), **Frostline** (6th combat map, frozen tundra), **weapon identity cards** (archetype + stat bars in the loadout), a **rising hitmarker**, and a **kill banner** ("ELIMINATED {name}"). Their overlapping post-match work was reconciled into one card (accolade + stat strip + NEW PERSONAL BEST). All pure-client / zero-protocol — solo + live MP both intact. Built on the v0.24.0 routine-integration round (Team Deathmatch, Onslaught, Cobalt + Overpass maps, LMG, melee, grenade, nameplates, bot difficulty), the publication round (site + game on Vercel, **MP server live on Fly.io** at `ilcartigo-game.fly.dev`, AdSense `ca-pub-8134911671778438` verified), and Phase 13–14 (Gun Game, Aim Lab, rank ladder, weapon mastery, Marksman, server-authoritative per-weapon damage, minimap, impact FX, health pickups, weapon finishes).
