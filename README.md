...
> **Status:** v0.46.0 — **prestige player skins.** A premium 7th skin tier for every class — **Eclipse Lord · Phoenix Lord · Aegis Lord · Phantom Lord · Forge Lord · Apex Lord** (6000 XP each) — the highest-visibility cosmetic (your character, seen by everyone in MP) and a real long-grind chase target (36 → 42 player skins). Pure data — the Cosmetics tab auto-iterates `SKINS`, and `Account`'s default-unlock + migration logic is untouched. Built on **v0.45.0 — "Took the lead" callout.** In FFA (solo + online), pulling strictly ahead of the field pops a "★ TOOK THE LEAD" toast + sting — a small drama beat on the kill-race that makes the standings feel alive and pushes "stay on top." Rising-edge detection in the HUD match ticker (strict lead, ≥1 kill; self-resets on a fresh match). Built on **v0.44.0 — King of the Hill zone callouts.** Taking the zone now pops a "⚑ ZONE CAPTURED" toast + sound and losing it pops "ZONE LOST", giving KotH the moment-to-moment juice the kill-based modes already had (control-flip detection in the `Hardpoint.onState` consumer; reset on match start). Built on **v0.43.0 — King of the Hill records + Profile bests polish.** KotH now has a persistent "improve" hook like the other solo modes: lifetime **wins** are saved (`ilc.koth.wins`), surfaced on the menu button ("⚑ King of the Hill · N wins") and in the **Profile → Bests** grid, which also now shows **Match Kills** (your best single-match kill count). Closes the progression loop on the newest mode. Pure-client, no protocol change. Built on **v0.42.0 — cosmetics content drop.** More to chase across all three XP-cosmetic axes (the Cosmetics UI auto-iterates the registries, so they appear / unlock / equip with no logic change): **+3 kill effects** (Frost Shatter · Golden Nova · Shadow Pulse → 11), **+2 tracers** (Sunset · Mint → 12), **+2 weapon finishes** (Obsidian · Prismatic → 10). Pure data — deepens the unlock grind (retention → ad impressions). Built on **v0.41.0 — Nemesis system.** In solo, the bot that kills you becomes your **nemesis** — flagged with a ☠ crimson nameplate + a pulsing red ring on the minimap so you can hunt it down. Avenging it (the kill that also pops the existing REVENGE callout) pays **+25 bonus XP** and a "☠ NEMESIS DOWN" toast — making the grudge concrete and feeding the "win the next duel" loop. Pure-client (`Game.nemesisId` set on death / cleared on the avenging kill + match reset), wired into Nameplates + Minimap; gated to solo (no bots/ids in MP). Built on **v0.40.0 — Weapon inspect (T).** A cosmetic twirl that pulls the gun in and turns it so the side faces the camera — a beauty shot for the skins/finishes players grind for (Krunker/CS staple). Pure viewmodel animation (`Viewmodel.playInspect`), pointer-lock gated, interrupted by fire/reload/swap/melee so it never fights combat. New `inspect` input (bound to `T`). Built on **v0.39.0 — Burst Rifle — 9th weapon.** A 3-round-burst battle rifle that rewards trigger discipline: one trigger pull fires a tight burst, `fireRate` governs the gap between bursts. A clean burst (3 body) leaves them just alive, so a headshot in the burst finishes the kill — the skill-expression rifle between the spray-happy AR and the one-tap Marksman. Implemented with new `WeaponConfig.burst`/`burstDelay` + `Weapon.tickBurst`/`cancelBurst` (Game drives the queued intra-burst shots each frame with live aim; swap/empty/reload abort the burst cleanly) — fully MP-safe (each burst shot is its own server-authoritative Fire event; new `SERVER_WEAPONS['burst']` + `VALID_WEAPONS` entry). Full client integration: config + battle-rifle viewmodel + archetype card + Gun Game label + loadout button + three mastery skins (Tactical / Ember / Royal). Verified with an 11-assertion headless firing test. Built on **v0.38.0 — Foundry — 7th combat map.** A molten steel-mill: dark plate-metal structures, a scorched floor, and glowing orange lava channels (decoration-only, non-colliding so the floor stays fully playable), warm-orange neon trim and a furnace-haze fog — the hottest-looking map, for instant contrast with the other six. Built on the proven symmetric Cobalt/Frostline skeleton (mirroring the verified collision geometry so every spawn is clear + TDM-fair), then fully re-themed. Solo-selectable across FFA / TDM / Gun Game / Onslaught / Duel / King of the Hill; health pads mirrored client+server; MP keeps serving its authoritative map so Foundry changes nothing online. New `maps/FoundryMap.ts`, registered in `MapId`/`MAPS`/menu/`COMBAT_MAPS`; geometry verified headlessly (all spawns clear, 8 jump pads). Built on **v0.37.0 — King of the Hill (Hardpoint) mode.** The first objective mode: a glowing capture zone sits on the map and you bank capture points by holding it — but the FFA bots hunt you, so they crowd in and **CONTEST** the zone, turning every hold into a fight. The hill **relocates every ~26 s**, forcing constant repositioning instead of camping. First side (you vs the pooled enemy bots) to 100 capture points wins → a results card (ad breakpoint). Reuses the normal respawning combat roster verbatim (every frag still flows through XP / stats / killstreak rewards), overlaying only a scoring zone + win condition. New `modes/Hardpoint.ts`, `'koth'` GameMode, a 3D zone ring tinted by control, a HUD ticker (your score vs enemy + CONTESTED pill + relocation countdown), a minimap marker, and a "⚑ King of the Hill" menu button. Solo-only, pure-client, no protocol change — verified with a headless state-machine test (solo capture win, contested freeze, enemy win, relocation). Built on **v0.36.0 — killstreak rewards.** Hitting a streak milestone now pays out a tangible perk, so a hot streak is worth protecting and dying to lose it stings: **3 → RESUPPLY** (instant full mags), **5 → OVERCHARGE** (×1.7 dmg), **7 → FIELD MEDIC** (full heal), **10 → RAPID FIRE** (×1.55 RoF), **15 → SECOND WIND** (heal + resupply), **20 → OVERDRIVE** (everything). Reuses the arena-power-up weapon layer + buff timers (perks clear on death / fresh match), wired through a new `Announcer.onStreakMilestone` → `Game.grantStreakReward`; solo combat modes only (MP damage/HP stay server-authoritative). Pure-client, no protocol change. Built on **v0.35.0 — per-weapon reload animations + weapon colours.** Each weapon now plays a mechanism-appropriate reload motion timed to its real reload window — mag-swap cant/dip (AR · SMG · Marksman · LMG), bolt-cycle jerk (Sniper), double pump-rack (Shotgun), slide pull (Pistol), smooth power-cell tilt (Railgun) — layered into the viewmodel alongside recoil/bob/swap. The FBX weapon exports shipped with all-white materials, so colours are reconstructed from each part's material **name** (LightWood/Metal/Barrels…), plus a per-file index palette for models with generic names (the P90 → dark-olive body). Built on **v0.34.0 — real 3D weapon models**: the first-person viewmodel shows detailed FBX guns (rifle, P90, sniper, shotgun, LMG, ray-gun, pistol) instead of procedural boxes — lazy-loaded, de-rigged (SkinnedMesh→static Mesh so they clone + render), auto-normalized + oriented muzzle-forward, with a graceful box fallback. Built on the v0.33.0 **second routine-integration round.** Two more autonomous build branches merged onto `main`, combining everything they each built. **From the power-ups/progression branch:** **Arena Power-Ups** (OVERCHARGE ×1.7 dmg, RAPID FIRE ×1.55 RoF, OVERSHIELD 50%-absorb — contested buff pads, solo-only), the **Railgun** (8th weapon — piercing beam, one-shot heads, line multi-kills), **Daily Login Rewards** (7-day escalating XP streak), **"ON FIRE" rampage** (persistent killstreak aura), **cosmetics expansion** (kill effects →8, tracers →10, finishes →8), and **skill-shot callouts** (NO SCOPE / AIRBORNE / LONGSHOT). **From the content/feel branch:** **Duel** (solo 1v1 gauntlet — single-elimination ladder vs escalating rivals), **Frostline** (6th combat map, frozen tundra), **weapon identity cards** (archetype + stat bars in the loadout), a **rising hitmarker**, and a **kill banner** ("ELIMINATED {name}"). Their overlapping post-match work was reconciled into one card (accolade + stat strip + NEW PERSONAL BEST). All pure-client / zero-protocol — solo + live MP both intact. Built on the v0.24.0 routine-integration round (Team Deathmatch, Onslaught, Cobalt + Overpass maps, LMG, melee, grenade, nameplates, bot difficulty), the publication round (site + game on Vercel, **MP server live on Fly.io** at `ilcartigo-game.fly.dev`, AdSense `ca-pub-8134911671778438` verified), and Phase 13–14 (Gun Game, Aim Lab, rank ladder, weapon mastery, Marksman, server-authoritative per-weapon damage, minimap, impact FX, health pickups, weapon finishes).
