...
## Breacher — auto-shotgun (v0.40.0)

A ninth weapon for the loadout — the brief's weapon-variety pillar. The
**Breacher** is a fully-automatic combat shotgun: a fast stream of lighter
pellets (7 × 8 = 56 close, 3.6 RPS, 10-round drum, 2.6 s reload) that wins
close-quarters duels through *sustained DPS* instead of the pump's single
decisive blast — but a wide cone + brutal 5→18 m falloff make it useless past
brawling range. A distinct hold-the-trigger CQB archetype.

- Reuses the existing pellet + automatic fire paths (no engine change). Full
  integration: `BREACHER_CONFIG` + `WEAPON_LIBRARY`, a chunky `buildBreacher` box
  viewmodel (no FBX model → graceful box fallback), a `pump` reload animation,
  `WEAPON_LABEL` (Gun Game) + "Auto Shotgun" archetype, a loadout button, three
  mastery skins (Riot / Hazard / Crimson). The compiler-enforced
  `Record<WeaponId,…>` maps all updated.
- **MP-safe:** a `SERVER_WEAPONS['breacher']` profile (30 single-ray dmg — the
  server models spread as one ray) + a `VALID_WEAPONS` entry. No protocol change.

## Neon District — new combat map (v0.39.0)

The sixth combat map — fresh content deepens **every** combat mode at once
(FFA / TDM / Gun Game / Onslaught / Duel / **Hardpoint** + the map selector).
Distinct identity: **a cyberpunk night district.**

- **Neon District** — near-black reflective streets laced with glowing neon grid
  lines, holographic translucent magenta cover panels (solid for collision +
  hitscan, visually airy), alternating cyan/magenta emissive trim, deep-night sky
  + neon-haze fog. The darkest/flashiest map for instant contrast with the five
  others.
- **Built on the proven symmetric Cobalt/Frostline skeleton** (TDM-fair, spawn
  corners known-clear) then fully re-themed: raised neon central platform
  (jump-pad ring + holo pillar + corner cover), two N/S team decks with parapets
  (magenta-N / cyan-S side identity), diagonal holo-panel cover, E/W flank walls,
  low steppable kerbs. Verticality entirely jump-pad driven.
- **Solo-selectable, zero MP risk.** New `maps/NeonMap.ts` registered in
  `MapId`/`MAPS` + a loadout button + the `COMBAT_MAPS` list; health pads
  mirrored `maps/Pickups.ts` ⇆ `server/src/Pickups.ts`. Geometry verified
  headlessly (all 6 spawns clear of solids). No protocol/server change.
