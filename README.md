...
> **Status:** v0.41.0 — **menu unlock-chase line.** The next-unlock teaser now also shows on the **main menu** (the most-viewed screen): a compact "Next unlock: {name} {category} · {N} XP to go" line under the rank badge, refreshed on boot + every XP gain/unlock. Shared `computeNextUnlock()` backs both the menu line and the post-match teaser. Pure client, no protocol change. Built on v0.40.0 — **prestige cosmetics tier.** Six new high-XP unlocks (kill effects Prismatic 6k / Supernova 8k, tracers Prism 4.5k / Phoenix 6k, finishes Prismatic 5.5k / Obsidian Gold 7k) extend the grind ceiling for dedicated players — long-term goals that pair with the new next-unlock teaser. Pure data (the Cosmetics UI auto-iterates the registries), no logic change. Built on v0.39.0 — **"Next Unlock" teaser.** The post-match overlay (the game's main natural ad breakpoint) now ends on a progression hook: it scans every XP-gated cosmetic axis (skins / kill effects / tracers / finishes), finds the cheapest item you haven't unlocked yet, and shows its name, category, a progress bar, and exact XP-to-go — the "one more game and it's yours" pull that keeps players (and the ad screen) engaged. Computed after match XP is awarded, so it reflects your new total. Pure client, no protocol change. Built on v0.38.0 — **combat audio depth.** Three feedback cues that round out the v0.35 sound layer: a **reload-complete chime** (only when the mag actually fills — interrupted reloads stay silent), a **weapon-switch clack** when you swap slots, and a spatial **near-miss bullet whiz** when incoming fire passes within ~3 m of your head without hitting you (Krunker-style "that was close" threat awareness — works for bots in solo and remotes in MP). All synthesized, pure client, no protocol change. Built on v0.37.0 — **the Crossbow (9th weapon).** A silent, pinpoint skill weapon: very slow (1.1 RPS), 2-bolt mag, but a **one-shot body kill** (100 dmg, ×2 head) at any range — miss and you're caught reloading, land it and they're gone. All-config (no new fire mechanic) so it's MP-safe like the LMG/Railgun: client config + procedural viewmodel + a `SERVER_WEAPONS` profile + `VALID_WEAPONS` entry, loadout button, Gun Game label, weapon-stat card archetype, 3 mastery skins, and a quiet bowstring-thwip synth shot. Built on v0.36.0. — **synthesized music.** A new `MusicEngine` plays a calm minor-key **menu theme** and a driving **combat track** — both generated in real time via a Web Audio lookahead scheduler (no audio files), with their own volume slider + on/off toggle in Settings → Audio. Tracks switch automatically: menu/results screens get the theme, gameplay gets the combat pulse. Built on the v0.35.0 audio engine. — **the game finally has sound.** Every SFX (8 weapons, hitmarkers, the rising-hitmarker headshot chain, kill confirms, footsteps, jumps, ability casts, grenade/railgun zaps, announcer multi-kill + streak stings, level-up, pickups) is now **synthesized in real time** by a new Web Audio `SynthEngine` — so the game ships **fully audible with zero asset files**, closing the single biggest gap to Krunker's satisfying-shooting / flashy-hit-feedback feel. The drop-in `.wav` pipeline is preserved (drop a file in `/client/public/assets/sounds/` + list its id in `FILE_BACKED` in `AudioManager.ts` → that one sound loads from disk, the rest stay synthesized). Spatial cues (remote gunfire, footsteps) pan + fall off through the synth too. Pure client, no protocol change — solo + MP both intact. Built on the v0.34.0 **real 3D weapon models.** The first-person viewmodel now shows detailed FBX gun models (rifle, P90, sniper, shotgun, LMG, ray-gun, pistol) instead of the procedural box-guns — lazy-loaded, de-rigged (SkinnedMesh→static Mesh so they clone + render), auto-normalized to a consistent in-hand size, with a graceful box fallback if a model is missing. All existing viewmodel behaviour (recoil, walk-bob, swap dip, muzzle-flash anchor, cloak fade, weapon-finish emissive) preserved. Built on the v0.33.0 **second routine-integration round.** Two more autonomous build branches merged onto `main`, combining everything they each built. **From the power-ups/progression branch:** **Arena Power-Ups** (OVERCHARGE ×1.7 dmg, RAPID FIRE ×1.55 RoF, OVERSHIELD 50%-absorb — contested buff pads, solo-only), the **Railgun** (8th weapon — piercing beam, one-shot heads, line multi-kills), **Daily Login Rewards** (7-day escalating XP streak), **"ON FIRE" rampage** (persistent killstreak aura), **cosmetics expansion** (kill effects →8, tracers →10, finishes →8), and **skill-shot callouts** (NO SCOPE / AIRBORNE / LONGSHOT). **From the content/feel branch:** **Duel** (solo 1v1 gauntlet — single-elimination ladder vs escalating rivals), **Frostline** (6th combat map, frozen tundra), **weapon identity cards** (archetype + stat bars in the loadout), a **rising hitmarker**, and a **kill banner** ("ELIMINATED {name}"). Their overlapping post-match work was reconciled into one card (accolade + stat strip + NEW PERSONAL BEST). All pure-client / zero-protocol — solo + live MP both intact. Built on the v0.24.0 routine-integration round (Team Deathmatch, Onslaught, Cobalt + Overpass maps, LMG, melee, grenade, nameplates, bot difficulty), the publication round (site + game on Vercel, **MP server live on Fly.io** at `ilcartigo-game.fly.dev`, AdSense `ca-pub-8134911671778438` verified), and Phase 13–14 (Gun Game, Aim Lab, rank ladder, weapon mastery, Marksman, server-authoritative per-weapon damage, minimap, impact FX, health pickups, weapon finishes).
...
**As of v0.35.0 every sound is synthesized in real time** by `client/src/audio/SynthEngine.ts`, so the game is fully audible out of the box with **no asset files**. The `.wav` pipeline below is now an *optional override*: drop a CC0 `.wav` into `client/public/assets/sounds/` matching one of these names **and add its id to `FILE_BACKED` in `client/src/audio/AudioManager.ts`** — that single sound then loads from disk while everything else keeps using the synth. (Before v0.35.0 a missing file was silently skipped; now it's synthesized instead.)
