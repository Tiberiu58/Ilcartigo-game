...
> **Status:** v0.44.0 — **first win of the day bonus.** Your first match **win** each local day grants a one-time **+200 XP · +50 🪙** on top of the normal rewards, surfaced as a pulsing gold "★ FIRST WIN OF THE DAY" badge on the post-match overlay (the main ad breakpoint) and folded into the displayed XP/coin totals. The classic "come back and win once" daily hook — pulls players into at least one full match per day (→ retention → ad impressions → coins feeding the shop). Migration-safe `Account` extension (`firstWinDate` + `claimFirstWinOfDay`), one claim per day, works in solo and MP wins. **No protocol change.** Built on **v0.43.0 — killstreak buff rewards.** Going on a tear now *pays off mechanically*: at escalating solo killstreaks (**5, 9, 13, 17…** — every 4 kills) you're auto-granted a temporary power-up, cycling **RAPID FIRE → OVERCHARGE → OVERSHIELD**, with a "KILLSTREAK n — REWARD" callout + the existing buff feedback (SFX, screen flash, buff-tray pill). It reuses the arena power-up plumbing (`Game.grantPowerup`) and is gated exactly like the pads — **solo combat only** (not MP, where damage is server-authoritative, nor Gun Game / Practice) — and the buff clears on death with the streak, so the reward rises and falls with your run. Stacks with the existing "ON FIRE" rampage aura for a real snowball-the-fight feel. Pure-client, **no protocol change.** Built on **v0.42.0 — cosmetics drop (71-item shop catalogue).** +12 cosmetics — **4 kill effects** (Golden Shock · Rose Burst · Azure Nova · Spectral Rift), **4 bullet tracers** (Rose · Azure · Spectral · Sunset), **4 weapon finishes** (Rose Gold · Azure · Spectral · Onyx) — bringing the coin-purchasable catalogue to **71 items**. Pure data: the Cosmetics tab and the daily Shop both auto-iterate the registries, so the new items appear, unlock, equip and rotate into the shop with no logic change — more to chase, save up for, and rotate through daily (→ more sessions, more ad impressions). **No protocol change.** Built on **v0.41.0 — Meridian (8th combat map).** A sunlit white-marble & gold coliseum — warm marble ground, sandy-stone structures, gold neon trim under a midday sun, for instant contrast with warm Sandstone, rusty Industrial, steel Cobalt, dusk Overpass, frozen Frostline and molten Foundry. Built on the proven symmetric Cobalt/Frostline skeleton (TDM-fair, spawns collision-identical to the verified layout) then fully re-themed marble + gold with opaque stone-column cover. Solo-selectable across all 9 modes (FFA · TDM · Gun Game · Onslaught · Duel · King of the Hill); health pads mirrored client+server; MP keeps serving its authoritative map, so Meridian is solo-only and changes nothing online. New `maps/MeridianMap.ts`. **No protocol change.** Built on **v0.40.0 — King of the Hill (zone-control mode).** A fresh objective type — instead of racing to a kill count you fight to **hold a contested zone** at the arena's heart. Stand in it alone and your capture meter fills (HOLDING, teal); let a bot in and it's **CONTESTED** (amber, nobody scores); cede it and the enemy pool fills (LOSING HILL, red); first side to **100** wins → the standard post-match overlay (an ad breakpoint). It rewards positioning + map control, not just aim — the push-and-shove "take the point, hold the point" loop. Runs **on top of the live combat bot roster** (the 3 auto-respawning base bots keep the hill contested) so every frag still feeds XP/coins/stats/announcer; death → normal auto-respawn. A translucent capture column + pulsing ground ring (status-coloured) marks the hill, with a live HUD ticker (YOU vs ENEMY + status). New `modes/KingOfTheHill.ts`, `'koth'` GameMode, **♛ King of the Hill** menu button. Pure-client, **no protocol change**. Built on **v0.39.0 — coins economy + daily shop.** A new soft currency (🪙 coins) is earned in every match — **+2 per kill, +25 per win / +12 top-3**, plus bonus coins from daily-login and daily-challenge claims — and spent in a new **🛒 Shop**: a daily-rotating, deterministic-per-day lineup of **6 cosmetic offers** (skins · kill effects · tracers · finishes, drawn from the 59-item paid catalogue) with one discounted **FEATURED** deal. Buying unlocks **and** auto-equips the cosmetic — a second, grind-free path to the same unlocks that the XP-cost Cosmetics tab offers, plus a "save up for the thing you want" collect loop and a daily reason to return (→ more menu ad impressions). Coin balance shows on the main menu; the shop opens from a menu button + overlay (a natural ad-adjacent moment). Pure-client, migration-safe `Account` extension, **no protocol change**. New `account/Shop.ts` (catalogue + seeded daily generator) + `ui/ShopUI.ts`. Built on **v0.38.0 — third routine-integration round + the game has SOUND.** Cherry-picked 11 distinct features off the routine's parallel branches onto current `main`, verifying typecheck + build green after each: **procedural audio engine** (`SynthEngine.ts` synthesizes all 48 SFX at runtime via Web Audio — gunshots, hits, reloads, UI — no `.wav` files; the game was silent before this), **Burst Rifle** (9th weapon, 3-round burst), **Foundry** (7th combat map, molten steelworks), **career achievements + 24 medals** (Awards tab + toasts), **Nemesis system** (marked rival + revenge bonus), **Prestige "Lord" skins** (6000-XP 7th tier), **weapon inspect (T)**, **killfeed weapon icons**, **Final Blow** win cinematic (slow-mo), **hit-combo meter** (x3+ chain counter), and **enemy hit-flash** on landed shots. Skipped the duplicate King-of-the-Hill / neon-map / extra-9th-weapon lines (two branches built them incompatibly); the Crossbow was dropped (heavy conflicts). All v0.34–37 model/animation work (incl. the first-person arms fix) preserved. Built on **v0.37.0 — first-person arms.** The viewmodel now has two low-poly arms whose hands grip the held weapon — a right (trigger) hand + left (foregrip) hand, positioned per-weapon via grip anchors (two-handed near the rear for the pistol). They bob/recoil/swap with the gun and add a **walk/idle sway** of their own — a stronger stride sway that ramps with speed, plus a gentle idle breath when still. Cloak fade includes the hands. Built on **v0.36.0 — animated player character (multiplayer).** Remote MP players now render as a rigged low-poly humanoid (CubeMan) instead of the cyan box-figure, with **driven animations** — Idle when still, Run when moving (from interpolated speed), Death on elimination — via a per-instance `AnimationMixer` and `SkeletonUtils.clone` (the rig must be preserved + properly rebound, unlike the de-rigged guns). Lazy-loaded (FBX + SkeletonUtils in their own chunks; the 3.2 MB model only fetches in MP), with the box figure as a fallback until ready; cloak fade preserved. Bots keep the box figure (per design — characters for *players* only). Built on **v0.35.0 — per-weapon reload animations + weapon colours.** Each weapon now plays a mechanism-appropriate reload motion timed to its real reload window — mag-swap cant/dip (AR · SMG · Marksman · LMG), bolt-cycle jerk (Sniper), double pump-rack (Shotgun), slide pull (Pistol), smooth power-cell tilt (Railgun) — layered into the viewmodel alongside recoil/bob/swap. The FBX weapon exports shipped with all-white materials, so colours are reconstructed from each part's material **name** (LightWood/Metal/Barrels…), plus a per-file index palette for models with generic names (the P90 → dark-olive body). Built on **v0.34.0 — real 3D weapon models**: the first-person viewmodel shows detailed FBX guns (rifle, P90, sniper, shotgun, LMG, ray-gun, pistol) instead of procedural boxes — lazy-loaded, de-rigged (SkinnedMesh→static Mesh so they clone + render), auto-normalized + oriented muzzle-forward, with a graceful box fallback. Built on the v0.33.0 **second routine-integration round.** Two more autonomous build branches merged onto `main`, combining everything they each built. **From the power-ups/progression branch:** **Arena Power-Ups** (OVERCHARGE ×1.7 dmg, RAPID FIRE ×1.55 RoF, OVERSHIELD 50%-absorb — contested buff pads, solo-only), the **Railgun** (8th weapon — piercing beam, one-shot heads, line multi-kills), **Daily Login Rewards** (7-day escalating XP streak), **"ON FIRE" rampage** (persistent killstreak aura), **cosmetics expansion** (kill effects →8, tracers →10, finishes →8), and **skill-shot callouts** (NO SCOPE / AIRBORNE / LONGSHOT). **From the content/feel branch:** **Duel** (solo 1v1 gauntlet — single-elimination ladder vs escalating rivals), **Frostline** (6th combat map, frozen tundra), **weapon identity cards** (archetype + stat bars in the loadout), a **rising hitmarker**, and a **kill banner** ("ELIMINATED {name}"). Their overlapping post-match work was reconciled into one card (accolade + stat strip + NEW PERSONAL BEST). All pure-client / zero-protocol — solo + live MP both intact. Built on the v0.24.0 routine-integration round (Team Deathmatch, Onslaught, Cobalt + Overpass maps, LMG, melee, grenade, nameplates, bot difficulty), the publication round (site + game on Vercel, **MP server live on Fly.io** at `ilcartigo-game.fly.dev`, AdSense `ca-pub-8134911671778438` verified), and Phase 13–14 (Gun Game, Aim Lab, rank ladder, weapon mastery, Marksman, server-authoritative per-weapon damage, minimap, impact FX, health pickups, weapon finishes).
