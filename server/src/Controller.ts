...
export class ServerController {
  ...
  speedMultiplier = 1.0;
  /** Haste power-up multiplier — mirror of the client field so the two movement
   *  controllers stay identical. Power-ups are solo-only / client-side, so this
   *  stays 1.0 server-side; present purely to keep the math in sync. */
  ...
  step(dt: number, input: PlayerInput) {
    ...
    if (this.grounded) {
      ...
      if (hasInput && wishSpeed > 0) this.accelerate(wx, wz, wishSpeed * this.speedMultiplier * this.powerupSpeedMultiplier, GROUND_ACCEL, dt);
