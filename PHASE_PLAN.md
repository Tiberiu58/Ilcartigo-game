...
### Status log (cont.)
- ✅ Phase 13C — Armor/overshield model + armor pickups. DONE + headless-verified (client Health absorb math: +50→30dmg→armor20/hp100→40dmg→armor0/hp80; server armor pad claim grants 50; server+client tsc + build green). Overshield absorbs 1:1 before HP, gained only via armor pads, zeroed on respawn, no regen. Inserted at `Health` (client) so bots are unaffected; mirrored in `ServerPlayer.armor` + onFire absorb. `PlayerSnapshot.armor` syncs the local HUD bar in MP. One armor pad/map (Sandstone NE rooftop, Industrial yard centre) at index 4/3, lockstep in client meta + server `PICKUPS_BY_MAP`. New thin blue HUD armor bar above HP (hidden at 0).
- ✅ Phase 13D — MP max-HP HUD fix + crosshair presets. DONE (server+client tsc + build green). `PlayerSnapshot.maxHp` so the local HP bar normalises to the real max online (Vanguard 115 no longer 100-capped); reconcile reads it. One-click crosshair presets (Dot/Small/Cross/Large) drive the existing CSS vars + persist + sync sliders. (TDM was scoped out: bots only target the player — no bot-vs-bot for a solo variant — and MP needs a server game-mode concept that isn't menu-selectable in a single-server deploy; left in the post-v1 list with the blockers noted.)
- ✅ Phase 13E — README + version bump + polish. DONE. Bumped client+server to v0.13.0 (+ menu subtitle/footer), README Phase 13 section + audio catalog additions (pickup_health/armor/ammo), status + deliverables + post-v1 list updated. All typecheck + build green.
